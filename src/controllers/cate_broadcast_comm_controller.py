...
class CateBCommMAC:
    ...
    def forward(self, ep_batch, t, test_mode=False):
        ...
        if self.args.comm:
            (mu, sigma), logits, messages, m_sample = self._communicate(ep_batch.batch_size, agent_inputs)
        ...
        if self.args.comm:
            return agent_outs.view(ep_batch.batch_size, self.n_agents, -1, self.args.atom), (mu, sigma), logits, m_sample
        else:
    ...
    def _communicate(self, bs, inputs):
        ...
        return (mu.view(bs, self.n_agents, -1), sigma.view(bs, self.n_agents, -1)), logits, message.view(bs*self.n_agents, -1), ms
